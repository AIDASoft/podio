...
ArrowWriter::ArrowWriter(const std::string& directory, Options options)
...
ArrowWriter::ArrowWriter(const std::string& directory)
    : ArrowWriter(directory, Options{}) {
}
...
void ArrowWriter::writeFrame(const podio::Frame& frame, std::string_view category, const std::vector<std::string>& collsToWrite) {
...
void ArrowWriter::validateSchema(CategoryInfo& catInfo, const podio::Frame& frame, const std::vector<std::string>& collsToWrite) {
  ...
  for (size_t i = 0; i < collsToWrite.size(); ++i) {
    ...
    if (catInfo.collTypes[i] != coll->getValueTypeName() ||
        catInfo.collIsSubset[i] != coll->isSubsetCollection() ||
        catInfo.collSchemaVersions[i] != coll->getSchemaVersion() ||
        catInfo.collIDs[i] != coll->getID()) {
...
void ArrowWriter::flushCategory(CategoryInfo& catInfo) {
  ...
  if (!catInfo.writer) {
    ...
    auto writer_result = parquet::arrow::FileWriter::Open(*catInfo.schema, arrow::default_memory_pool(), outfile, builder.build(), arrow_props);
    if (!writer_result.ok()) {
...
void ArrowWriter::writeMetadata() {
  ...
  for (const auto& [name, def] : m_datamodelCollector.getDatamodelDefinitionsToWrite()) {
    ...
    if (edmVersion) {
      edmVersionsJson[name] = {{"major", edmVersion.value().major}, {"minor", edmVersion.value().minor}, {"patch", edmVersion.value().patch}};
    }
  }
