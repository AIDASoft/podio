...
CollectionIDTable::CollectionIDTable(std::vector<uint64_t>&& ids, std::vector<std::string>&& names) :
    m_collectionIDs(std::move(ids)), m_names(std::move(names)), m_mutex(std::make_unique<std::mutex>()) {
}

CollectionIDTable::CollectionIDTable(const std::vector<uint64_t>& ids, const std::vector<std::string>& names) :
    m_collectionIDs(ids), m_names(names), m_mutex(std::make_unique<std::mutex>()) {
}

const std::string CollectionIDTable::name(uint64_t ID) const {
...
uint64_t CollectionIDTable::collectionID(const std::string& name) const {
  std::lock_guard<std::mutex> lock(*m_mutex);
...
uint64_t CollectionIDTable::add(const std::string& name) {
  std::lock_guard<std::mutex> lock(*m_mutex);
  const auto result = std::find(begin(m_names), end(m_names), name);
  uint64_t ID = 0;
