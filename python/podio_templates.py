...
implementations["member_class_setter"] = "void {classname}::{fname}(class {type} value) {{ m_obj->checkAccess(); m_obj->data.{name} = value; }}\n"

# this is inline, don't need the declaration
implementations["const_member_getter"] =  "\t/// Access the {description}\n"
implementations["const_member_getter"] += "\tconst {type}& {classname}::{fname}() const {{ return m_obj->data.{name}; }}\n"
...
implementations["pod_member_builtin_setter"] = "void {classname}::{fname}({type} value){{ m_obj->checkAccess(); m_obj->data.{compname}.{name} = value; }}\n"
...
implementations["pod_member_class_setter"] = "void {classname}::{fname}(class {type} value) {{ m_obj->checkAccess(); m_obj->data.{compname}.{name} = value; }}\n"

# this is inline, don't need the declaration
implementations["const_pod_member_getter"] =  "\t/// Access the  member of {description}\n"
implementations["const_pod_member_getter"] = "\tconst {type}& {classname}::{fname}() const {{ return m_obj->data.{compname}.{name}; }}\n"

# OneToOneRelations
#-----------------------------------
declarations["one_rel_setter"] =  "\t/// Set the {description}\n"
declarations["one_rel_setter"] += "\tvoid {name}({namespace}::{type} value);\n"
#declarations["one_rel_setter"] += "\tvoid {name}({namespace}::{type} value);\n"
implementations["one_rel_setter"]  = "void {classname}::{name}({namespace}::{type} value) {{\n"
implementations["one_rel_setter"] += "\tif (m_obj->m_{name} != nullptr) delete m_obj->m_{name};\n"
implementations["one_rel_setter"] += "\tm_obj->m_{name} = new {type}(value);\n}}\n"

declarations["one_rel_getter"] =  "\t/// Access the {description}\n"
declarations["one_rel_getter"] += "\tconst {namespace}::{type} {name}() const;\n"
implementations["one_rel_getter"] = "\tconst {namespace}::{type} {classname}::{name}() const {{\n"
implementations["one_rel_getter"] += "\t\tif (m_obj->m_{name} == nullptr) {{\n"
implementations["one_rel_getter"] += "\t\t\treturn {namespace}::{type}(nullptr);\n"
implementations["one_rel_getter"] += "\t\t}}\n"
implementations["one_rel_getter"] += "\t\treturn {namespace}::{type}(*(m_obj->m_{name}));\n\t}}"
# this is inline, don't need the declaration
implementations["const_one_rel_getter"] =  "\t/// Access the {description}\n"
implementations["const_one_rel_getter"] += "\tconst {namespace}::{type} {classname}::{name}() const {{\n"
implementations["const_one_rel_getter"] += "\t\tif (m_obj->m_{name} == nullptr) {{\n"
implementations["const_one_rel_getter"] += "\t\t\treturn {namespace}::{type}(nullptr);\n"
implementations["const_one_rel_getter"] += "\t\t}}\n"
implementations["const_one_rel_getter"] += "\t\treturn {namespace}::{type}(*(m_obj->m_{name}));}}"
...
declarations["relation"] = "\tstd::vector<{namespace}::{type}>* m_rel_{name}; ///< Relation buffer for read / write\n"
declarations["relation_collection"] = "\tstd::vector<std::vector<{namespace}::{type}>*> m_rel_{name}_tmp; ///< Relation buffer for internal book-keeping\n"

implementations["ctor_list_relation"] = ", m_rel_{name}(new std::vector<{namespace}::{type}>())"
