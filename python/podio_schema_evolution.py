...
class RenamedDataType(SchemaChange):
  ...
  def __init__(self, name_old, name_new):
    ...
    super().__init__(f"'{self.name_new}': datatype '{self.name_old}' renamed to '{self.name_new}'.")
  ...
  def heuristics(self):
    ...
    for change in changed_members:
      ...
      if change.old_member.is_array != change.new_member.is_array:
        self.errors.append(f"Forbidden schema change in '{change.name}' for '{change.member_name}' from "
    ...
    dropped_datatypes = [change for change in schema_changes if isinstance(change, DroppedDatatype)]
    added_datatypes = [change for change in schema_changes if isinstance(change, AddedDatatype)]
    ...
    dropped_components = [change for change in schema_changes if isinstance(change, DroppedComponent)]
    added_components = [change for change in schema_changes if isinstance(change, AddedComponent)]
