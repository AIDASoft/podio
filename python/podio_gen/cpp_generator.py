...
class CPPClassGenerator(ClassGeneratorBaseMixin):
  ...
  def __init__(self, yamlfile, install_dir, package_name, io_handlers, verbose, dryrun, upstream_edm,
               old_description, evolution_file):
    ...
    for member in schema_evolution_datatype['Members']:
      if member.is_array:
        if member.array_type in self.root_schema_dict:
          ...
          replace_component_in_paths(member.array_type, member.array_type + self.old_schema_version,
        ...
        if member.full_type in self.root_schema_dict:
          ...
          replace_component_in_paths(member.full_type, member.full_type + self.old_schema_version,
                                     schema_evolution_datatype['includes_data'])
