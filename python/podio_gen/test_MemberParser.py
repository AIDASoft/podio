...
class MemberParserTest(unittest.TestCase):
    ...
    def test_parse_valid(self):  # pylint: disable=too-many-statements
        ...
        parsed = parser.parse(
            r"std::array<std::int16_t, 42> fixedWidthArray // a fixed width type array"
        )
        ...
        parsed = parser.parse(
            r"std::array<uint32_t, 42> fixedWidthArray // a fixed width type array without namespace"
        )
    ...
    def test_parse_valid_default_value(self):
        ...
        parsed = parser.parse(
            r"std::array<int, 25> array{1, 2, 3} // we do not have to init the complete array"
        )
        ...
        parsed = parser.parse(
            r"int weirdDefault{whatever, even space} // invalid c++ is not caught"
        )
        self.assertEqual(parsed.default_val, "whatever, even space")

        parsed = parser.parse(
            r"int floatInit{3.14f} // implicit conversions are not allowed in aggregate initialization"
        )
        self.assertEqual(parsed.default_val, "3.14f")
