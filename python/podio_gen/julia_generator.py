...
class JuliaClassGenerator(ClassGeneratorBaseMixin):
    ...
    def print_report(self):
        ...
        text = REPORT_TEXT_JULIA.format(
            yamlfile=self.yamlfile, nfiles=nfiles, installdir=self.install_dir
        )
    ...
    def post_process(self, datamodel):
        ...
        datamodel["static_arrays_import"] = self._has_static_arrays_import(
            datamodel["components"] + datamodel["datatypes"]
        )
        datamodel["includes"] = self._sort_components_and_datatypes(
            datamodel["components"] + datamodel["datatypes"]
        )
    ...
    def _sort_components_and_datatypes(data):
        """Sorts a list of components and datatypes based on dependencies,
        ensuring that components and datatypes with no dependencies or
        dependencies on built-in types come first. The function performs
        ...
        for component_data in data:
            ...
            if "VectorMembers" in component_data:
                for vector_member_data in component_data["VectorMembers"]:
                    ...
                    if (
                        not vector_member_data.is_builtin
                        and not vector_member_data.is_builtin_array
                    ):
        ...
        while dependencies:
            ...
            if not ready:
                sorted_components.extend(
                    bare_types_mapping[component] for component in dependencies
                )
                break
    ...
    def _has_static_arrays_import(data):
        """Checks if any member within a list of components and datatypes
        contains the import statement 'using StaticArrays' in its jl_imports.
        Returns True if found in any member, otherwise False.
