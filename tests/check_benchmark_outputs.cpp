...
bool verifyTree(TTree* tree, int expectedEntries, const StringVec& expectedBranches) {
  ...
  if (tree->GetEntries() != expectedEntries) {
    std::cerr << "Tree \'" << treeName << "\' should have " << expectedEntries << " but has " << tree->GetEntries() << std::endl;
    return false;
  }
  ...
  for (const auto& branch : expectedBranches) {
    ...
    if (!found) {
      std::cerr << "Branch \'" << branch << "\' was expected to be in Tree \'" << treeName << "\' but could not be found" << std::endl;
      return false;
    }
  }
...
int main(int, char* argv[]) {
  ...
  const StringVec readBMSetupBranches = {
