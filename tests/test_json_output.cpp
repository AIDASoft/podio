...
// Copyright (c) 2023 key4hep contributors
//
...
TEST_CASE("JSON output - plain POD members", "[json]") {
  ...
  REQUIRE(j["energy"] == Catch::Approx(3.14f).epsilon(1e-5));

  REQUIRE(j.contains("x"));
  REQUIRE(j["x"] == Catch::Approx(1.0));

  REQUIRE(j.contains("y"));
  REQUIRE(j["y"] == Catch::Approx(2.0));

  REQUIRE(j.contains("z"));
  REQUIRE(j["z"] == Catch::Approx(3.0));
...
TEST_CASE("JSON output - integer POD members", "[json]") {
  ...
  REQUIRE(j.is_object());
  REQUIRE(j["PDG"] == 211);
  REQUIRE(j["energy"] == Catch::Approx(1.0));
}
...
TEST_CASE("JSON output - VectorMembers serialize as arrays", "[json]") {
  ...
  REQUIRE(j["count"].size() == 3);
  REQUIRE(j["count"][0] == 10);
  REQUIRE(j["count"][1] == 20);
  REQUIRE(j["count"][2] == 30);
}
...
TEST_CASE("JSON output - integer VectorMembers keep numeric type", "[json]") {
  ...
  REQUIRE(j["count"].is_array());
  REQUIRE(j["count"].size() == 4);

  for (const auto& elem : j["count"]) {
    REQUIRE(elem.is_number_integer());
  }
}
...
TEST_CASE("JSON output - unset OneToOneRelation", "[json]") {
  ...
  const bool isSentinel =
      clusterRef.contains("collectionID") && clusterRef["collectionID"] == std::numeric_limits<uint32_t>::max();
