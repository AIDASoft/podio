...
using namespace H5;	
const H5std_string FILE_NAME("test_dummy.h5");
...
int main()
{
	...
	try{
		...
		hsize_t a_1[] = {2}; 
		CompType g1(sizeof(eic1));
		g1.insertMember("EventInfoData", HOFFSET(eic1, e), H5Tarray_create(H5T_NATIVE_INT, 1,a_1));
		g1.insertMember("Id", HOFFSET(eic1, id), PredType::NATIVE_INT);

		hsize_t a_2[] = {4}; 
		CompType g2(sizeof(eic2));
		g2.insertMember("EventInfoData", HOFFSET(eic2, e), H5Tarray_create(H5T_NATIVE_INT, 1,a_2));
		g2.insertMember("Id", HOFFSET(eic2, id), PredType::NATIVE_INT);
		...
		// General DATASET 1
		const hsize_t g_SIZE = 1; 
		hsize_t g_dim[] = {g_SIZE};
		DataSpace g1_space(RANK, g_dim);
		std::shared_ptr<DataSet> g1_dataset(new DataSet(file->createDataSet("General Dataset 1", g1, g1_space)));
		
		// General DATASET 1
		DataSpace g2_space(RANK, g_dim);
		std::shared_ptr<DataSet> g2_dataset(new DataSet(file->createDataSet("General Dataset 2", g2, g2_space)));
		...
		// Fill general DATASET 1
		eic1 g1_data;
		 
		g1_data.e[0].Number = 20;  	
		g1_data.e[1].Number = 21;  	
		g1_data.id = 100000000;
		
		eic2 g2_data;
		
		g2_data.e[0].Number = 22;  	
		g2_data.e[1].Number = 23;
		g2_data.e[2].Number = 24;
		g2_data.e[3].Number = 25;
		g2_data.id = 100000001;
		...
		// write general dataset to file
		g1_dataset->write(&g1_data, g1);	
		g2_dataset->write(&g2_data, g2);	
